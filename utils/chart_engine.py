...
def generate_industry_charts(df: pd.DataFrame, industry: str, file_name: str) -> str:
    """Generates industry-specific charts and returns markdown image embed links."""
    ...
    if defect_col and pd.api.types.is_numeric_dtype(df[defect_col]):
        ...
        ax.set_title('Defect Rate Distribution (%)', fontsize=11, fontweight='bold')
        ax.set_xlabel('Defect Rate (%)')
        ax.set_ylabel('Batch Count')
        ax.legend()
        plt.tight_layout()
        
        img_path = f"{chart_dir}/{base_name}_defect_distribution.png"
        plt.savefig(img_path, dpi=200)
        plt.close()
        markdown_embeds.append(f"![Defect Distribution](charts/{os.path.basename(img_path)})")

    # 2. Categorical Dependency / Concentration Risk Chart
    ...
    if cat_col:
        ...
        img_path = f"{chart_dir}/{base_name}_concentration_risk.png"
        plt.savefig(img_path, dpi=200)
        plt.close()
        markdown_embeds.append(f"![Concentration Risk](charts/{os.path.basename(img_path)})")
