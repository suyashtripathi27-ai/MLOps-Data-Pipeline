...
def calc_pricing_metrics(df):
    ...
    cost_col = first_column(df, ["cost", "cogs", "unit_cost", "manufacturing_cost"])
    
    if not selling_price_col:
        return kpis
    
    conf, warns = confidence_for(df, [col for col in [regular_price_col, selling_price_col, discount_col, cost_col] if col])
    
    if pd.api.types.is_numeric_dtype(df[selling_price_col]):
    ...
    if discount_col and pd.api.types.is_numeric_dtype(df[discount_col]):
        ...
        if not valid_discount.empty:
            ...
            discount_items = (valid_discount > 0).sum()
            discount_rate = (discount_items / len(df) * 100) if len(df) > 0 else 0
            
            kpis.append(safe_kpi(
                category="💰 Pricing",
                name="Avg Discount",
                value=f"${avg_discount:,.2f}" if valid_discount.max() > 100 else f"{avg_discount:.2f}%",
            ...
            kpis.append(safe_kpi(
                ...
                source=f"`{discount_col}`",
                confidence=conf,
                warnings="High discount rate" if discount_rate > 50 else warns
            ))
        ...
        if not margin_df.empty:
            ...
            if not valid_margins.empty:
                ...
                kpis.append(safe_kpi(
                    ...
                    source=f"`{cost_col}`, `{selling_price_col}`",
                    confidence=conf,
                    warnings="Low margin" if avg_margin < 20 else warns
                ))
    
    # Price elasticity (if regular and selling price both exist)
    if regular_price_col and selling_price_col and pd.api.types.is_numeric_dtype(df[regular_price_col]) and pd.api.types.is_numeric_dtype(df[selling_price_col]):
