...
def calc_cost_metrics(df):
    ...
    if len(df) == 0:
        return kpis
    cost_col = first_column(df, ["total_cost", "operating_cost", "ProductionCost", "AdditiveMaterialCost"])
    unit_cost_col = first_column(df, ["unit_cost", "cost_per_unit"])
    scrap_cost_col = first_column(df, ["scrap_cost", "waste_cost"])
