...
def calc_revenue_metrics(df, enable_debug=False):
    """
    Calculate revenue and growth KPIs with optional execution tracing.
    ...
    median_revenue = revenue_series.median()
    ...
    kpis.append(engine.build_kpi(
        ...
        value=f"${median_revenue:,.2f}", formula="Median(Revenue)", source=f"`{revenue_col}`"
    ))
    ...
    if segment_col is not None:
        ...
        if len(revenue_by_seg) > 0:
            ...
            warn_msg = "High concentration (>70%)" if top_segment_share > 70 else "None"
            kpis.append(engine.build_kpi(
