...
def calc_investment_metrics(df, enable_debug=False):
    ...
    if investment_col is not None:
        ...
        kpis.append(engine.log_missing("📊 Investment Portfolio", "Portfolio Value", "Missing numeric 'investment_value'."))
    ...
    if return_col is not None:
        ...
        kpis.append(engine.build_kpi(
            ...
            warnings="Negative returns" if min_return < 0 else "None"
        ))
    else:
        kpis.append(engine.log_missing("📊 Investment Portfolio", "Return Metrics", "Missing numeric 'return' column."))
    ...
    if allocation_col is not None:
        ...
        if num_allocations > 0:
            ...
            kpis.append(engine.build_kpi(
                ...
                source=f"`{allocation_col}`", warnings="Concentration risk (>50%)" if top_allocation_share > 50 else "None"
            ))
    else:
        kpis.append(engine.log_missing("📊 Investment Portfolio", "Asset Allocation", "Missing 'asset_type' column."))
